...
using Testeando.AngularJS.Migrations.SeedData;
...
namespace Testeando.AngularJS.Tests
{
    public abstract class AngularJSTestBase : AbpIntegratedTestBase<AngularJSTestModule>
    {
        ...
        protected void UsingDbContext(Action<AngularJSDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AngularJSDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AngularJSDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AngularJSDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AngularJSDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AngularJSDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AngularJSDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AngularJSDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AngularJSDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AngularJSDbContext, Task<T>> func)
        {
            T result;
